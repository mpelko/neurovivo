import numpy
def data(Chan, start=-1, stop=-1, timeunits='seconds', as_double=True):
	'''
	...
	if Chan.info.kind in [2, 3, 4]:
		...
		for block in range(startBlock, endBlock):
			...
			timings[pointer : pointer+blockheader[4, block]] = numpy.fromfile(fid, numpy.int32, count=blockheader[4, block])
			#\
	...
	#	+ Marker data +
		...
		for block in range(startBlock, endBlock):
			...
			for i in range(blockheader[4, block]):         # loop for each marker
				timings[count] = numpy.fromfile(fid,numpy.int32,count=1)
				markers[count] = numpy.fromfile(fid,numpy.int8,count=4)
	...
	#	+ AdcMark data +
		...
		for block in range(startBlock, endBlock):
			...
			for i in range(blockheader[4, block]):         # loop for each marker
				timings[count] = numpy.fromfile(fid,numpy.int32,count=1)
				markers[count] = numpy.fromfile(fid,numpy.int8,count=4)
				adc[count] = numpy.fromfile(fid,numpy.int16,count=nValues)
				# timings[count] = io.fread(fid, 1, 'l')     # time
				# markers[count] = io.fread(fid, 4, 'B')     # 4x marker bytes
	...
	#	+ RealMark data +
		...
		for block in range(startBlock, endBlock):
			...
			for i in range(blockheader[4, block]):         # loop for each marker
				timings[count] = numpy.fromfile(fid,numpy.int32,count=1)
				markers[count] = numpy.fromfile(fid,numpy.int8, count=4)
				real[count] = numpy.fromfile(fid,numpy.float32, count=nValues)
				# timings[count] = io.fread(fid, 1, 'l')     # time
				# markers[count] = io.fread(fid, 4, 'B')     # 4x marker bytes
	...
	#	+ TextMark data +
		...
		for block in range(startBlock, endBlock):
			...
			for i in range(blockheader[4, block]):         # loop for each marker
				timings[count] = numpy.fromfile(fid,numpy.int32,count=1)
				markers[count] = numpy.fromfile(fid,numpy.int8, count=4)
				text[count] = numpy.fromfile(fid,numpy.character,count=Chan.info.nExtra)
				# timings[count] = io.fread(fid, 1, 'l')     # time
				# markers[count] = io.fread(fid, 4, 'B')     # 4x marker bytes
