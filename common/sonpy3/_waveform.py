...
def data(Chan, start=-1, stop=-1, timeunits='seconds', as_float=True):
	'''
	...
	# =======================================================
	if Chan.info.kind == 1:   datatype = numpy.int16 # Adc channel, 'int16'
	elif Chan.info.kind == 9: datatype = numpy.float32 # RealWave channel, 'single'
...
#		raise ValueError, 'Start value should be bigger than 0'
	...
	if NumFrames == 1:		
		...
		for i in range(startEpoch, endEpoch):
			...
			data[pointer : pointer+blockheader[4, i]] = numpy.fromfile(fid,datatype,count=blockheader[4, i])
			# data[pointer : pointer+blockheader[4, i]] = io.fread(fid, blockheader[4, i], datatype)
		...
		for i in range(startEpoch, endEpoch):
			fid.seek(blockheader[0, i] + SizeOfHeader)
			data[index, pointer : pointer+blockheader[4, i]] = numpy.fromfile(fid,datatype,count=blockheader[4, i])
			#\
