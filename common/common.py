...
# -----------------------------------------------------------------------------
    '''
    tries to copy source file to destination file. Convinience method.
    '''
    import shutil
...
# -----------------------------------------------------------------------------
    '''
    The pwd in linux. Convinience method.
    '''
    return os.getcwd()
...
# -----------------------------------------------------------------------------
def save_metadata(PATH, metadata, method="pickle"):
# -----------------------------------------------------------------------------
    '''
    ...
    if method == "yaml":
        save_yaml(PATH + "/metadata/metadata.yml", metadata)
    elif method=="pickle":
        save_pickle(PATH + "/metadata/metadata.pkl", metadata)

# -----------------------------------------------------------------------------
def load_metadata(PATH, method="pickle"):
# -----------------------------------------------------------------------------
    '''
    ...
    if method == "yaml":
        try:
            md = load_yaml(PATH+"/metadata/metadata.yml")
        except:
            raise
            assert False, "metadata missing. Expeceted metadata.yml at {}".format(PATH+"/metadata/")
...
# -----------------------------------------------------------------------------
    ...
    save_pickle(PATH+"{}{}.pkl".format(name, info_string), result)
    return PATH+"{}{}.pkl".format(name, info_string)

# -----------------------------------------------------------------------------
def save_with_info_string_yaml(chosen_params, result, PATH, name="result"):
# -----------------------------------------------------------------------------
    """
    Given a path for results it saves them as a pickled dictionary in a file 
    with a formatted informative name based on the chosen_params.
    ...
    """
    info_string = create_info_string(chosen_params)
    save_yaml(PATH+"{}{}.pkl".format(name, info_string), result)
    return PATH+"{}{}.pkl".format(name, info_string)
...
# -----------------------------------------------------------------------------
    ...
    keys.sort()
    for key in keys:
        info_string = "~".join([info_string,"{}-{}".format(key,chosen_params[key])])
    return info_string
...
# -----------------------------------------------------------------------------
    ...
    for file_name in all_file_names:
        ...
        for part in parts[1:]:
            ...
            # special case if the value has a - sign
            if len(part.split("-")) == 3:
                [key, miss, value] = part.split("-")
                factor = -1
                assert miss == "", "strange part ({}) of a file name ({})".format(part, file_name)
        ...
        for i in params.keys():
            # the file without the key in the name is not a good one
            if not i in file_params.keys():
                got_one=False
                break
            # strings get treated differently then numbers
            if isinstance(params[i], str) or isinstance(params[i], types.NoneType):
                if not params[i]==file_params[i]:
                    got_one=False
                    break
            elif not isinstance(file_params[i], str) or not isinstance(file_params[i], types.NoneType):
...
# -----------------------------------------------------------------------------
    ...
    dictionary = yaml.load(input_file)
    input_file.close()
    
    return dictionary
        
# -----------------------------------------------------------------------------
def load_results_yaml(PATH_RESULTS, chosen_params):
# -----------------------------------------------------------------------------
    """
    Given a path with files that represent results for different parameter
    values it loads and returns the result file (a dictionary).
    """
    file_name = get_chosen_file_name(PATH_RESULTS, chosen_params)
    return load_yaml(PATH_RESULTS+"/"+file_name)
...
# -----------------------------------------------------------------------------
    ...
    if not metadata is None:
        ...
        metadata_str=pp.pformat(metadata)
        pp.pprint(metadata)
...
# -----------------------------------------------------------------------------
def splice_two_vectors(vec1,vec2,borders):
# -----------------------------------------------------------------------------
    '''
    Returns only the parts of vec1 and vec2 for indices for which vec1 is 
    withing the borders. vec1 is presumed to be monotonically increasing 
    function.
    '''
    import bisect
    bord_ind = [bisect.bisect_left(vec1,borders[0]),bisect.bisect_right(vec1,borders[1])]
    return vec1[bord_ind[0]:bord_ind[1]], vec2[bord_ind[0]:bord_ind[1]]

# -----------------------------------------------------------------------------
def OU_process(theta, mu, sigma, times, IC=None, np_rnd=np.random):
# -----------------------------------------------------------------------------
    '''
    Returns the path according to the Ornstein-Uhlenbeck process. IC is 
    initial condition and equals mu if not specified.
    '''
    ...
    y[0] = IC
    for i in xrange(1,N):
        y[i] = y[i-1] + dt*(theta*(mu-y[i-1])) + sigma*sqrtdt*np_rnd.normal(0,1)    
    
    return y

# -----------------------------------------------------------------------------
def OU_process_basic(theta, times, np_rnd=np.random):
# -----------------------------------------------------------------------------
    '''
    Returns the path according to the Ornstein-Uhlenbeck process - basic. 
    The std of the process is presumed to be 1.
    '''
    ...
    y = np.zeros(N)
    y[0]=np_rnd.normal(0,1)
    for i in xrange(1,N):
        y[i] = y[i-1] + dt*(theta*(-y[i-1])) + sigma*sqrtdt*np_rnd.normal(0,1)    
    return y

# -----------------------------------------------------------------------------
def OU_process_intuative(time_constant, times, np_rnd=np.random):
# -----------------------------------------------------------------------------
    '''
    Returns the path according to the Ornstein-Uhlenbeck process. 
    The std of the process is presumed to be 1.
    '''
    
    theta = 1./time_constant
    return OU_process_basic(theta, times, np_rnd)

# -----------------------------------------------------------------------------
def OU_process_basic_generator(theta, dt, np_rnd=np.random):
# -----------------------------------------------------------------------------
    '''
    Returns the generator to the Ornstein-Uhlenbeck process - basic. 
    The std of the process is presumed to be 1.
    '''
    ...
    y=np_rnd.normal(0,1)
    while True:
        yn = y + dt*(theta*(-y)) + sigma*sqrtdt*np_rnd.normal(0,1)    
        y=yn
        yield y
        
# -----------------------------------------------------------------------------
def OU_process_intuative_generator(time_constant, dt, np_rnd=np.random):
# -----------------------------------------------------------------------------
    '''
    Returns the generator to the Ornstein-Uhlenbeck process - basic intuative. 
    The std of the process is presumed to be 1.
    '''
...
# -----------------------------------------------------------------------------
def lognormal_paramters(mean, std):
# -----------------------------------------------------------------------------
    """
    Given a desired mean and standard deviation, it returns the mu and sigma 
    parameters defining the lognormal distributon. 
    """
    mu = np.log(mean)-1./2*np.log(1+std**2/mean**2)
