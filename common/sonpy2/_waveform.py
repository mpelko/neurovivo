...
def data(Chan, start=-1, stop=-1, timeunits='seconds', as_float=True):
	...
	'''
	from scipy import array, histogram, zeros
	import numpy as np
	fid = Chan.fhead.fid
...
#		raise ValueError, 'Start value should be bigger than 0'
	...
	if NumFrames == 1:		
		...
		for i in range(startEpoch, endEpoch):
			...
			data[pointer : pointer+blockheader[4, i]] = np.fromfile(fid, datatype, blockheader[4, i])# io.fread(fid, blockheader[4, i], datatype)
		...
		for i in range(startEpoch, endEpoch):
			...
			data[index, pointer : pointer+blockheader[4, i]] =\
					np.fromfile(fid, datatype, blockheader[4, i])	
